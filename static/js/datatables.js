...
$.createTableMenu = function (init) {
        ...
        function createColorPicker() {				
			...
			handleDialogCancel= function() {this.cancel();}
			$.oColorPickerDialog = new YAHOO.widget.Dialog("yui-picker-panel", { 
				...
				zindex: 1002,
				constraintoviewport : true,
				buttons : [{ text:"Exit", handler:this.handleDialogCancel } ]			
             });
		...
		function colorPickerButtonClick(sColor){
					...
					context.ace.callWithAce(function(ace){ace.ace_doDatatableOptions(selParams);},'tblOptions' , true);
        ...
        $.getTblPropertiesHTML = function () {		
            return "<span id='table_properties'><span class='tbl-prop-menu-header'></span><br><span id='tbl-prop-menu'class='tbl-prop-menu'>" +
					"<table class='left-tbl-props tbl-inline-block'>" +
					"<tr><td class='tbl-prop-label-td'><span class='tbl-prop-label' style='padding-top: 8px;'>Table border</span></td></tr>" +
  ...
  $.tblContextMenu = new YAHOO.widget.ContextMenu("tbl_context_menu", {
            ...
            $("td", "#matrix_table").click(function (e) {
                context.ace.callWithAce(function(ace){ace.ace_doDatatableOptions('addTbl','addTblX' + $("#select_matrix").text());},'tblOptions' , true);
                return false;
            });
			...
			function initTableProperties(){
				...
				$('.text-input').change(function () {
					var selParams = {tblPropertyChange: true};				
  ...
  $('#tbl-menu').click($.createTableMenu); 
   ...
   YAHOO.util.Dom.addClass(document.body,'yui-skin-sam');
	$("body").append($('<div id="yui-picker-panel" class="yui-picker-panel">'+
						'<div class="hd">Please choose a color:</div>'+
						'<div class="bd">'+
						'	<div class="yui-picker" id="color-picker-menu"></div>'+
						'</div>'+						
						'<div class="ft"></div>'+
						'</div>'));
	$("body").append($('<div id="yui-tbl-prop-panel" class="yui-picker-panel">'+
						'<div class="hd">Table/Image Properties</div>'+
						'<div class="bd">'+
						'	<div class="yui-picker" id="tbl-props"></div>'+
						'</div>'+						
						'<div class="ft"></div>'+
						'</div>'));
...
{
	// Iterate through the child nodes (spans) and tabularize them
...
if (typeof (Datatables) == 'undefined') var Datatables = function () {
	 ...
	 dt.doDatatableOptions = function(cmd,xByY){
		...
		if((typeof(cmd)=='object'&&cmd.tblPropertyChange)){
			...
			switch (cmd) {         	
			     case Datatables.vars.TBL_OPTIONS[0]:Datatables.addTable(xByY);break;         
			     case Datatables.vars.TBL_OPTIONS[1]:Datatables.insertTblRow("addA");break;
			     case Datatables.vars.TBL_OPTIONS[2]:Datatables.insertTblRow("addB");break;         
			     case Datatables.vars.TBL_OPTIONS[3]:Datatables.insertTblColumn("addL");break;
			     case Datatables.vars.TBL_OPTIONS[4]:Datatables.insertTblColumn("addR");break;         
			     case Datatables.vars.TBL_OPTIONS[5]:Datatables.deleteTable();break;         
			     case Datatables.vars.TBL_OPTIONS[6]:Datatables.deleteTblRow();break;         
			     case Datatables.vars.TBL_OPTIONS[7]:Datatables.deleteTblColumn();break;
			    }
		}
	};
	...
	dt.deleteTblRow = function() {
	   ...
	   dt.updateTableProperties  = function(props){
		...
		if(props.tblWidth||props.tblHeight||props.tblBorderWidth||props.tblBorderColor
			||props.tblColWidth||props.tblSingleColBgColor||props.tblEvenRowBgColor||props.tblOddRowBgColor||props.tblColVAlign){		
			var start = [];
			start[0]=rep.selStart[0],start[1]=rep.selStart[1];
   ...
   dt.addColumnAttr = function(start,tblJSONObj,tblProperties,attrName,attrValue,currTd){
	...
	dt.updateAuthorAndCaretPos = function(magicDomLineNum,tblRowNum,tblColNum){
	 ...
	 dt.createDefaultTblProperties =  function(authors){
		var rep = this.context.rep;
		var defTblProp = {borderWidth:"1",cellAttrs:[],width:"6",rowAttrs:{},colAttrs:[],authors:{}};
		if(authors){
	...
	dt.performDocApplyTblAttrToRow = function(start,jsoStrTblProp){
			...
			this.context.editorInfo.ace_performDocumentApplyAttributesToRange(tempStart, tempEnd, [["tblProp", jsoStrTblProp]]);
	}
	...
	dt.getTdInfo = function(payload,tdIndex){
		...
		for(var tIndex = 0,tLen  = tds.length;tIndex<tLen;tIndex++){
			...
			payloadSum += tds[tIndex].length+overHeadLen;			
			if(tIndex>=tdIndex){ 				
				return {cellStartOffset:payloadSum - tds[tIndex].length - overHeadLen,
				cellEndOffset:payloadSum
				}
			}
		}
	...
	dt.updateTblPropInAPool = function (row, td, jsoTblProp, start) {
		...
		var thisAuthor = editorInfo.ace_getThisAuthor();
	 ...
	 dt.getCurrTblOddEvenRowBgColor = function(startRowNum,currRowNum){
		var rowBgColors={oddBgColor:null,evenBgColor:null};
	...
	dt.getTblAboveRowsFromCurFocus = function(start) {
	 ...
	 dt.updateTableIndices = function(tblProperties, currTd, addOrDel) {	
		...
		if (addOrDel == 'add') {//insert column            
				if(cellAttr){cellAttr.splice(currTd, 0, null);}			
		} else { //remove column            
				if(cellAttr){cellAttr.splice(currTd, 1);}
		}
	    }
		...
		colAttrs  = tblProperties.colAttrs;
		 if (addOrDel == 'add') {//insert column
			if(colAttrs){colAttrs.splice(currTd, 0, null);}
		}
		else{//remove column
			if(colAttrs){colAttrs.splice(currTd, 1);}
		}
	    return tblProperties;
	};
